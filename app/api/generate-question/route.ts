...
export async function POST(req: NextRequest) {
  // Wrap the ENTIRE handler in try/catch so that if the Anthropic SDK throws
  // (e.g. invalid API key, rate limit, network error), we still return a valid
  // JSON error response instead of letting Next.js return an HTML error page.
  // An HTML response causes the client's res.json() to throw "Unexpected end of JSON input".
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      // Increased from 512 → 1024. Fintech questions with SPEI/Bre-B/regulatory
      // context can be long. 512 tokens was enough to truncate mid-JSON.
      max_tokens: 1024,
      system: USER_PROFILE,
      messages: [
        {
...
Use this exact format:
{
  ...
  "tips": "A one-sentence coaching tip for how to approach this question (e.g., 'Think about a specific launch you owned and quantify the impact.')"
    ...
    // Extract text, then strip any markdown code fences Claude might add
    // despite the instruction (e.g. ```json ... ```)
    ...
    return NextResponse.json(
      { error: `Failed to generate question: ${msg}` },
      { status: 500 }
    );
  }
}
